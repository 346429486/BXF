...
class CarouselViewCell : UICollectionViewCell {
    ...
    var carouselModel : CarouselModel? {
        didSet {
            imageView.cz_setImage(urlString: carouselModel?.picUrl, placeholderImage:UIImage(named: ""))
...
extension CarouselView : UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        // 2.设置pageControl
        pageControl.currentPage = Int(offsetX / scrollView.bounds.width) % carouselVM.carousels.count
    }
